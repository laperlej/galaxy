...
class UniverseApplication(StructuredApp, GalaxyManagerApplication):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if self.config.enable_oidc:
            ...
            self.config.dont_show_preference = self.config.enable_oidc and (
                len(list(self.config.oidc)) == 1 and len(list(self.auth_manager.authenticators)) == 0
            )
            self.config.fixed_delegated_auth = self.config.fixed_delegated_auth and self.config.dont_show_preference
